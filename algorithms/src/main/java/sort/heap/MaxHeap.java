...
/**
 * 使用动态数组实现最大堆, 这里使用jdk中ArrayList实现
...
public class MaxHeap<E extends Comparable<E>> {
	...
	public MaxHeap(int capacity){
		...
		// 由于定义堆的下标是从1开始的, 所以data的第一个元素设置为null
		data.add(null);
	}
	...
	public MaxHeap(Collection<E> collection){
		...
		data.add(0, null);
		for(int i = parent(data.size() -1); i >= 1; i--){
			siftDown(i);
		}
	}
	...
	private void siftUp(int index){
		...
		while (index > 1 && data.get(parent(index)).compareTo(data.get(index)) < 0) {
